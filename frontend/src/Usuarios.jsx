  ...
  return (
    <div className="mobile-scroll overflow-y-auto h-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tighter text-white">Usuarios</h1>
      ...
      {feedback && (
        <div className={`mb-4 text-sm rounded-xl p-3 border ${
          feedback.type === 'success' ? 'bg-emerald-500/10 border-emerald-200 text-emerald-700' :
          feedback.type === 'error' ? 'bg-red-500/10 border-red-200 text-red-700' :
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {lista.map(u => (
            <div key={u.id} className="bg-[#141414] rounded-2xl p-5 shadow-[0_6px_16px_rgba(0,0,0,0.03)] border border-[#262626]">
              {editId === u.id ? (
                <form onSubmit={guardar} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block mb-1">Rol</label>
                    ...
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-2 text-sm text-[#D4D4D8]">
                        <input type="checkbox" checked={editForm.activo === 1}
                          ...
                          className="rounded border-[#71717A] text-white focus:ring-slate-900/20" />
                        Activo
                      </label>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className={`text-[10px] font-semibold px-2.5 py-0.5 rounded-full ${
                        u.rol === 'admin' ? 'bg-[#0A0A0A] text-white' :
                        u.rol === 'juridico' ? 'bg-blue-100 text-blue-700' :
                        'bg-[#262626] text-[#D4D4D8]'
                      }`}>
                        {ROL_LABEL[u.rol] || u.rol}
                      </span>
                      {!u.activo && (
                        <span className="text-[10px] font-semibold bg-red-500/10 text-red-400 px-2 py-0.5 rounded-full">Inactivo</span>
                      )}
                      {u.id === currentUser?.id && (
                        <span className="text-[10px] text-[#A1A1AA]">(tú)</span>
                      )}
                    </div>
                  ...
                  {currentUser?.rol === 'admin' && (
                    <button onClick={() => abrirEdicion(u)}
                      className="text-[#71717A] hover:text-[#D4D4D8] transition-colors text-sm px-2 py-1 rounded-lg hover:bg-[#262626] shrink-0 ml-4">
                      ✎
                    </button>
                  )}
                </div>
