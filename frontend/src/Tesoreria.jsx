...
export default function Tesoreria({ usuario }) {
  ...
  const renderCuentas = () => (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-slate-900">Cuentas bancarias</h2>
      ...
      {showCuentaForm && (
        <form onSubmit={handleCrearCuenta} className="bg-white rounded-2xl p-6 mb-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Nueva cuenta bancaria</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Banco</label>
            ...
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Saldo inicial</label>
      ...
      {cuentas.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <div className="text-4xl mb-3">🏦</div>
          <p className="text-sm text-slate-400">No hay cuentas registradas</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {cuentas.map(c => (
            <div key={c.id} className="bg-white rounded-2xl p-5 shadow-[0_6px_16px_rgba(0,0,0,0.03)] border border-slate-900/5 hover:shadow-[0_12px_32px_rgba(0,0,0,0.06)] transition-all">
    ...
    <div>
      <div className="flex items-center gap-4 mb-4 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Cuenta</label>
        ...
        <div className="pt-5">
          <button onClick={() => { setShowMovForm(!showMovForm); setError(''); }}
            ...
            className="bg-slate-900 text-white text-xs font-semibold rounded-xl px-4 py-3 hover:bg-slate-800 transition-all disabled:opacity-40">
      ...
      {showMovForm && (
        <form onSubmit={handleCrearMovimiento} className="bg-white rounded-2xl p-6 mb-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Nuevo movimiento</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Fecha</label>
            ...
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Concepto</label>
            ...
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Monto</label>
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-400 text-sm">Cargando movimientos...</div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left py-3 px-2 text-slate-400 font-medium">Concepto</th>
                  <th className="text-right py-3 px-2 text-slate-400 font-medium">Monto</th>
                  <th className="text-center py-3 px-4 text-slate-400 font-medium">Conciliado</th>
                </tr>
              </thead>
              <tbody>
                {movimientos.map(m => (
                  <tr key={m.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    <td className="py-3.5 px-4 text-slate-600">{fmtFecha(m.fecha)}</td>
                    <td className="py-3.5 px-2">
                      <span className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                        m.tipo === 'abono' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                      }`}>
                        {m.tipo === 'abono' ? 'Abono' : 'Cargo'}
                      </span>
                    </td>
                    <td className="py-3.5 px-2 text-slate-900">{m.concepto}</td>
                    <td className={`text-right py-3.5 px-2 font-semibold ${
                      m.tipo === 'abono' ? 'text-emerald-600' : 'text-red-600'
                    }`}>
                      {fmt(m.monto)}
                    </td>
        ...
        <div className="bg-white rounded-2xl p-12 text-center shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <p className="text-sm text-slate-400">Selecciona una cuenta para ver sus movimientos</p>
        </div>
      )}
    </div>
  );
  ...
  const renderConciliacion = () => (
    <div>
      <h2 className="text-base font-semibold text-slate-900 mb-4">Conciliación bancaria</h2>

      <form onSubmit={handleConciliar} className="bg-white rounded-2xl p-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Mes</label>
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Año</label>
            <input type="number" min={2020} max={2050} value={concAnio}
              onChange={e => setConcAnio(parseInt(e.target.value))}
              className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 text-sm outline-none focus:border-[#2E8B57] focus:ring-2 focus:ring-[#2E8B57]/15" />
          </div>
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Saldo estado de cuenta</label>
      ...
      {concResult && (
        <div className="bg-white rounded-2xl p-6 mt-4 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Resultado de conciliación</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-slate-50 rounded-xl p-4 text-center">
              <div className="text-lg font-extrabold text-slate-900">{fmt(concResult.saldo_sistema)}</div>
              <div className="text-[10px] text-slate-400 mt-1">Saldo en sistema</div>
            </div>
            <div className="bg-slate-50 rounded-xl p-4 text-center">
              <div className="text-lg font-extrabold text-slate-900">{fmt(concResult.saldo_estado_cuenta)}</div>
              <div className="text-[10px] text-slate-400 mt-1">Saldo estado de cuenta</div>
            </div>
            <div className={`rounded-xl p-4 text-center ${
              parseFloat(concResult.diferencia) === 0 ? 'bg-emerald-50' : 'bg-amber-50'
            }`}>
              <div className={`text-lg font-extrabold ${
                parseFloat(concResult.diferencia) === 0 ? 'text-emerald-600' : 'text-amber-600'
              }`}>
          ...
          {parseFloat(concResult.diferencia) === 0 && (
            <div className="mt-3 text-center text-xs text-emerald-600 font-semibold bg-emerald-50 rounded-xl p-3">✅ Cuenta conciliada exitosamente</div>
          )}
          {parseFloat(concResult.diferencia) !== 0 && (
            <div className="mt-3 text-center text-xs text-amber-600 font-semibold bg-amber-50 rounded-xl p-3">⚠️ Hay diferencias por revisar</div>
          )}
        </div>
      )}
    </div>
  ...
  const renderEstadoCuenta = () => (
    <div>
      <h2 className="text-base font-semibold text-slate-900 mb-4">Estado de cuenta</h2>

      <div className="bg-white rounded-2xl p-6 mb-4 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
      ...
      {loadingEc && <div className="text-center py-8 text-slate-400 text-sm">Cargando estado de cuenta...</div>}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-slate-50 rounded-xl p-4 text-center">
                <div className="text-lg font-extrabold text-slate-900">{fmt(ecData.saldo_inicial)}</div>
                <div className="text-[10px] text-slate-400 mt-1">Saldo inicial</div>
              </div>
              <div className="bg-slate-50 rounded-xl p-4 text-center">
                <div className="text-lg font-extrabold text-emerald-600">{fmt(ecData.cargos || 0)}</div>
                <div className="text-[10px] text-slate-400 mt-1">Cargos</div>
              </div>
              <div className="bg-slate-50 rounded-xl p-4 text-center">
                <div className="text-lg font-extrabold text-red-600">{fmt(ecData.abonos || 0)}</div>
                <div className="text-[10px] text-slate-400 mt-1">Abonos</div>
              </div>
              <div className="bg-slate-50 rounded-xl p-4 text-center">
                <div className="text-lg font-extrabold text-slate-900">{fmt(ecData.saldo_final)}</div>
                <div className="text-[10px] text-slate-400 mt-1">Saldo final</div>
              </div>
            </div>
          </div>
          ...
          {ecData.movimientos && ecData.movimientos.length > 0 && (
            <div className="bg-white rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 overflow-hidden">
              <div className="p-6 pb-0">
                <h3 className="text-sm font-semibold text-slate-900">Movimientos del período</h3>
              </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {ecData.movimientos.map(m => (
                      <tr key={m.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                        <td className="py-3 px-4 text-slate-600">{fmtFecha(m.fecha)}</td>
                        ...
                        <td className="py-3 px-2 text-slate-900">{m.concepto}</td>
