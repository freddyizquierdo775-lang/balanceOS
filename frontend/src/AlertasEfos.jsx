...
export default function AlertasEfos({ usuario }) {
  ...
  const renderDashboard = () => (
    <div>
      {loading ? (
        <div className="text-center py-12 text-slate-400 text-sm">Cargando dashboard...</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl p-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 text-center">
            <div className="text-3xl mb-2">🚨</div>
            <div className="text-3xl font-extrabold text-slate-900">{alertasActivas}</div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1">Alertas resueltas</p>
          </div>
          <div className="bg-white rounded-2xl p-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 text-center">
            <div className="text-3xl mb-2">📋</div>
            <div className="text-3xl font-extrabold text-slate-900">{listas.length}</div>
            <p className="text-xs text-slate-500 mt-1">RFCs en listas</p>
          </div>
        </div>
      )}
      ...
      {!loading && alertasActivas === 0 && alertasResueltas === 0 && (
        <div className="mt-4 bg-white rounded-2xl p-12 text-center shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <div className="text-4xl mb-3">🛡️</div>
          <p className="text-sm text-slate-400">Sin alertas registradas</p>
          <p className="text-xs text-slate-300 mt-1">Verifica clientes contra listas EFOS para generar alertas</p>
        </div>
      )}
    </div>
  );
    ...
    <div>
      ...
      {showListaForm && (
        <form onSubmit={handleAgregarLista} className="bg-white rounded-2xl p-6 mb-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Agregar RFC a lista</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">RFC</label>
              <input type="text" required value={listaForm.rfc}
                ...
                className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 text-sm uppercase outline-none focus:border-[#2E8B57] focus:ring-2 focus:ring-[#2E8B57]/15 placeholder:text-slate-300 placeholder:normal-case" />
            </div>
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">Tipo de lista</label>
      ...
      {listas.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          ...
          <p className="text-sm text-slate-400">No hay RFCs registrados en listas EFOS</p>
        </div>
      ) : (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left py-3 px-4 text-slate-400 font-medium">RFC</th>
                  <th className="text-left py-3 px-2 text-slate-400 font-medium">Tipo lista</th>
                  <th className="text-left py-3 px-2 text-slate-400 font-medium">Fecha publicación</th>
                  <th className="text-left py-3 px-4 text-slate-400 font-medium">Fecha consulta</th>
                </tr>
              </thead>
              <tbody>
                {listas.map(l => (
                  <tr key={l.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    <td className="py-3.5 px-4 text-slate-900 font-mono font-semibold">{l.rfc}</td>
                    <td className="py-3.5 px-2">
                      <span className="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-50 text-amber-700">
                        {l.tipo_lista}
                      </span>
                    </td>
                    <td className="py-3.5 px-2 text-slate-600">{fmtFecha(l.fecha_publicacion)}</td>
                    <td className="py-3.5 px-4 text-slate-600">{fmtFecha(l.fecha_consulta || l.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  const renderVerificar = () => (
    <div>
      <div className="mb-6">
        <h2 className="text-base font-semibold text-slate-900 mb-2">Verificar clientes contra listas EFOS</h2>
        <p className="text-xs text-slate-500">Revisa los RFCs de todos tus clientes contra las listas del SAT (69, 69-B, definitivos y sentencias)</p>
      </div>

      <button onClick={handleVerificarTodos} disabled={verificando}
        className="bg-slate-900 text-white text-sm font-semibold rounded-xl px-6 py-3 hover:bg-slate-800 transition-all disabled:opacity-50 mb-6">
        {verificando ? (
      ...
      {verificacionResultados && (
        <div className="space-y-4">
          {/* Resumen */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white rounded-2xl p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 text-center">
              <div className="text-2xl font-extrabold text-slate-900">{verificacionResultados.total || verificacionResultados.verificados || 0}</div>
              <div className="text-[10px] text-slate-400 mt-1">Clientes verificados</div>
            </div>
            <div className="bg-white rounded-2xl p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 text-center">
              <div className="text-2xl font-extrabold text-red-600">{verificacionResultados.en_lista || 0}</div>
              <div className="text-[10px] text-slate-400 mt-1">Encontrados en listas</div>
            </div>
            <div className="bg-white rounded-2xl p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 text-center">
              <div className="text-2xl font-extrabold text-emerald-600">{verificacionResultados.limpios || 0}</div>
              <div className="text-[10px] text-slate-400 mt-1">Sin incidencias</div>
            </div>
          </div>
          ...
          {verificacionResultados.resultados && verificacionResultados.resultados.length > 0 && (
            <div className="bg-white rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5 overflow-hidden">
              <div className="p-6 pb-0">
                <h3 className="text-sm font-semibold text-slate-900">Resultados detallados</h3>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-slate-100">
                      ...
                      <th className="text-center py-3 px-2 text-slate-400 font-medium">En lista</th>
                  ...
                  <tbody>
                    {verificacionResultados.resultados.map((r, i) => (
                      <tr key={i} className={`border-b border-slate-50 hover:bg-slate-50/50 ${
                        ...
                        <td className="py-3 px-4 text-slate-900 font-mono font-semibold">{r.rfc}</td>
                        <td className="py-3 px-2 text-slate-600">{r.cliente || r.nombre || '—'}</td>
                        ...
                        <td className="py-3 px-4 text-slate-600">{r.tipo || r.tipo_lista || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {!verificacionResultados && !verificando && (
        <div className="bg-white rounded-2xl p-12 text-center shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-slate-900/5">
          <div className="text-4xl mb-3">🔍</div>
          <p className="text-sm text-slate-400">Presiona el botón para verificar todos los clientes</p>
          <p className="text-xs text-slate-300 mt-1">Se revisarán RFCs contra listas 69, 69-B, definitivos y sentencias</p>
        </div>
      )}
    </div>
  );
  ...
  const renderAlertas = () => (
    <div>
      <h2 className="text-base font-semibold text-slate-900 mb-4">Alertas activas</h2>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left py-3 px-2 text-slate-400 font-medium">RFC</th>
                  ...
                  <th className="text-center py-3 px-4 text-slate-400 font-medium">Acción</th>
                </tr>
              </thead>
              <tbody>
                {alertas.map(a => (
  ...
  return (
    <div className="mobile-scroll overflow-y-auto h-full">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-white rounded-xl p-1 shadow-[0_4px_12px_rgba(0,0,0,0.03)] border border-slate-900/5 w-fit flex-wrap">
        {tabs.map(t => (
